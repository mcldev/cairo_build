...
mv builds/unix/freetype-config $RPM_BUILD_ROOT/%{_bindir}/freetype-config
{
  for ftdemo in ftbench ftdump ftlint ftvalid ; do
      builds/unix/libtool --mode=install install -m 755 ft2demos-%{version}/bin/$ftdemo $RPM_BUILD_ROOT/%{_bindir}/$ftdemo
  done
}
%if %{with_xfree86}
{
...
%doc docs/CHANGES docs/VERSIONS.TXT docs/formats.txt docs/ft2faq.html
